...
def get_new_token():
    ...
    r_text = BeautifulSoup(r.content, "html.parser").find(
        "script", {"id": "config"}).get_text()
    return json.loads(r_text)['accessToken']


def get_tracks(playlist_id, offset, limit, token):
    url = "https://api.spotify.com/v1/playlists/" + \
        str(playlist_id) + "/tracks?offset=" + str(offset) + \
        "&limit=" + str(limit) + "&market=GB"
    payload = {}
    headers = {
    ...
    while not done:
        ...
        if(data['total'] > 0):
            ...
            for song in data['items']:
                ...
                tracks.append(
                    {'name': song_name, 'artist': artist_name, 'song_image': song_image})
        else:
...
def download_playlist(spotify_playlist_id, folder_name):
    ...
    legal_path_characters = string.ascii_letters + \
        string.digits + " ()[]"  # Allowed characters in file path
    folder_name = ''.join(
        current_character for current_character in folder_name if current_character in legal_path_characters)
    ...
    for index, song in enumerate(songs):
        ...
        song_name = "".join([current_character for current_character in song["name"]
                            if current_character in legal_path_characters])
        artist = "".join([current_character for current_character in song['artist']
                         if current_character in legal_path_characters])
        ...
        song_final_dest = "downloads/" + \
            str(folder_name) + "/" + str(search_query) + '.mp3'

        if os.path.exists(song_final_dest):
            print(
                f"{bcolors.WARNING}Song {search_query} already available at {song_final_dest} skipping {bcolors.ENDC}")
            skipped_songs += 1
            continue

        print('\n' * 3)
        print(bcolors.CGREENBG + bcolors.CBLACK + f'Downloading song {index}/ {len(songs)}[ ' + str(
            song_name) + ' - ' + str(artist) + ' ]' + bcolors.ENDC + '\n')
        item_loc = 'downloads/' + str(spotify_playlist_id) + '/' + (
            (search_query + '.mp3').replace('"', '').replace("'", '').replace('\\', '').replace('/', ''))
        ...
        if(os.path.isfile(item_loc)):
            ...
            try:
                yt_results = YoutubeSearch(
                    search_query, max_results=1).to_json()

                yt_data = json.loads(yt_results)['videos'][0]

                print('View Count: ' + bcolors.UNDERLINE +
                      yt_data['views'] + bcolors.ENDC)
                print('Duration: ' + bcolors.UNDERLINE +
                      yt_data['duration'] + bcolors.ENDC + '\n')
                ...
                if(song_duration >= MAX_LENGTH):
                    print(bcolors.CREDBG + bcolors.CBLACK +
                          'Skipped - Song is longer than set max song length.' + bcolors.ENDC)
                    print(bcolors.CVIOLETBG2 + bcolors.CBLACK +
                          'Change MAX_LENGTH in the script to prevent skipping' + bcolors.ENDC)
                    raise ConfigException(
                        'Skipped song due to MAX_LENGTH value in script')

                if(song_viewcount <= MIN_VIEW_COUNT):
                    print(bcolors.CREDBG + bcolors.CBLACK +
                          'Skipped - Top song has low view count.' + bcolors.ENDC)
                    print(bcolors.CVIOLETBG2 + bcolors.CBLACK +
                          'Change MIN_VIEW_COUNT in the script to prevent skipping' + bcolors.ENDC)
                    raise ConfigException(
                        'Skipped song due to MIN_VIEW_COUNT value in script')
                ...
                print(bcolors.OKCYAN + ">   Downloaded mp4 without frames to " +
                      yt_tmp_out + bcolors.ENDC + '\n')

                urllib.request.urlretrieve(song_image_url, song_image_path)

                print(bcolors.OKCYAN + ">   Downloaded image album cover to " +
                      yt_tmp_out + bcolors.ENDC + '\n')
                ...
                audiofile.tag.images.set(ImageFrame.FRONT_COVER, open(
                    song_image_path, 'rb').read(), 'image/jpeg')
                audiofile.tag.save()

                shutil.copy(song_mp3_tmp_loc, song_final_dest)

                print(bcolors.OKGREEN + "Saved final file to " +
                      song_final_dest + bcolors.ENDC + '\n')
                ...
                if(not isinstance(e, ConfigException)):
                    ...
                    f.close()
                    print(
                        f"{bcolors.WARNING}Failed to convert {search_query} due to error.{bcolors.ENDC}. See failed_log.txt for more information.")
                    failed_downloads += 1
                    failed_song_names = failed_song_names + "\t• " + \
                        song_name + " - " + artist + \
                        f" | Fail reason: {e}" + "\n"
                    print(
                        'Please report this at https://github.com/couldbejake/spotify2mp3' + bcolors.ENDC)
                    quit()
                else:
                    f = open('failed_log.txt', 'a')
                    f.write(search_query + '\n' + str(e))
                    ...
                    f.close()
                    print(
                        f"{bcolors.WARNING}Failed to convert {search_query} due to config error.{bcolors.ENDC}. See failed_log.txt for more information.")
                    failed_downloads += 1
                    failed_song_names = failed_song_names + "\t• " + \
                        song_name + " - " + artist + \
                        f" | Fail reason: {e}" + "\n"
...
def main(spotify_url_link=None):
    ...
    if spotify_url_link == None:
        # '7rutb883T7WE7k6qZ1LjwU'
        spotify_url_link = input('\nEnter the spotify URL link: ')
    ...
    if not playlist_name:  # Dynamically determine playlist name
        ...
        page = requests.get(
            f"https://open.spotify.com/playlist/{spotify_url_link}")
        playlist_name = html.fromstring(page.content).xpath(
            '/html/body/div/div/div/div/div[1]/div/div[2]/h1')[0].text_content().strip()
